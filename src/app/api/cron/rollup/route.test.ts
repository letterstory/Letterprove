...
vi.mock("@/rollup/prune", () => ({ pruneHotEvents: vi.fn() }));
...
async function mocks() {
  return {
    rollupHotEvents: vi.mocked(
      (await import("@/rollup/sessions")).rollupHotEvents,
    ),
    pruneHotEvents: vi.mocked((await import("@/rollup/prune")).pruneHotEvents),
...
describe("GET /api/cron/rollup", () => {
  ...
  beforeEach(async () => {
    ...
    (await mocks()).pruneHotEvents.mockResolvedValue({ ok: true, deleted: 0 });
  });
  ...
  it("fails closed with 401 when CRON_SECRET is unset", async () => {
    ...
    expect((await mocks()).pruneHotEvents).not.toHaveBeenCalled();
  });
  ...
  it("returns 500 and pages with the scope named when the rollup fails", async () => {
    const { GET, rollupHotEvents, sendAlert } = await mocks();
    rollupHotEvents.mockResolvedValue({
      ok: false,
      detail: "rollup_hot_events_hourly: deadlock detected",
    });
  ...
  it("prunes raw events on every run and reports the count", async () => {
    const { GET, rollupHotEvents, pruneHotEvents } = await mocks();
    rollupHotEvents.mockResolvedValue({ ok: true });
    pruneHotEvents.mockResolvedValue({ ok: true, deleted: 7 });

    const res = await GET(request("Bearer test-secret"));

    expect(pruneHotEvents).toHaveBeenCalledOnce();
    expect(await res.json()).toEqual({
      ok: true,
      prune: { ok: true, deleted: 7 },
    });
  });

  // Retention is independent of the rollup: its rows are weeks outside the
  // rollup's window, so a failed rollup must not also leave raw data piling up.
  it("still prunes when the rollup fails", async () => {
    const { GET, rollupHotEvents, pruneHotEvents } = await mocks();
    rollupHotEvents.mockResolvedValue({ ok: false, detail: "boom" });
  ...
  // A failed delete pages, but must not make Vercel retry a rollup that worked.
  it("pages on a failed prune without failing the run", async () => {
    const { GET, rollupHotEvents, pruneHotEvents, sendAlert } = await mocks();
    rollupHotEvents.mockResolvedValue({ ok: true });
    pruneHotEvents.mockResolvedValue({
      ok: false,
      detail: "permission denied for function prune_hot_events",
    });
    ...
    expect(sendAlert).toHaveBeenCalledWith(
      ...
      "permission denied for function prune_hot_events",
    );
  });
  ...
  it("pages instead of crashing when the prune throws", async () => {
    const { GET, rollupHotEvents, pruneHotEvents, sendAlert } = await mocks();
    rollupHotEvents.mockResolvedValue({ ok: true });
    pruneHotEvents.mockRejectedValue(new Error("socket hang up"));
