 ...
 *
 * These mocks used to resolve to `{ id: "c1" }`. That was fine while nothing
 * inspected the payload, but dispatchTool now validates every success against
 * the tool's advertised outputSchema outside production — so a stub that
 * doesn't resemble a real row fails, correctly: a test asserting on a shape the
 * handler can never produce is testing the mock, not the code.
 */
